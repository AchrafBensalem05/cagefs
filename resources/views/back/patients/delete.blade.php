                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <form  method="post"  action="{{ route("patients.destroy" , $patient->id) }}">
                            @csrf
                            @method("DELETE")
                            <span>@lang("Are you sure from deleting") <strong>{{ $patient->title }}</strong> ?</span>
                            <hr>
                            <a class=" btn btn-success" href="{{ route("patients.index") }}"><i class="fa fa-arrow-left"></i>  @lang("Cancel")</a>
