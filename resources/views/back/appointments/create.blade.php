        ...
        <div class="row">
            <div class="col-10 offset-1">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form method="post" action="{{ route("appointment.store" , $type) }}" enctype="multipart/form-data">
                            @csrf
                            <div class="row">
                                        ...
                                        <select  class="select2bs4 form-control"  name="patient_id"  id="">
                                            ...
                                            @if($patients->count())
                                                @foreach($patients as $patient)
                                                    <option get-patient-info="{{ route('patients.get_patient_info', $patient->id) }}" value="{{ $patient->id }}">{{ $patient->name }}</option>
                                                @endforeach
                                            @endif
                                        </select>
                                    </div>
                                    <input type="text" class="form-control patient-name" placeholder="patient name ..." name="name" style="display: none">
                                    <small for="">Anonyme ?</small>
                                    <input type="checkbox"  name="anonym" >
                                </div>
                                <div class="col-md-6 col-sm-12 pb-2">
                                    <label for="">Phone number</label>
                                    <input type="tel" class="form-control" placeholder="phone number ..." name="phone" style="">
                                </div>
                                <div class="col-md-6 col-sm-12 pb-2">
                                    ...
                                    <select required class="select2bs4 form-control"  name="healthcare_entity_id"  id="">
                                        @if($healthcare_entities->count())
                                            ...
                                            @foreach($healthcare_entities as $entity)
                                                <option get-services-url="{{ route('healthcareEntity.get_services_of_healthcare_entity' , $entity->id) }}" value="{{ $entity->id }}">{{ $entity->name }} | {{ \App\Models\HealthcareEntity::Types[$entity->type] }}</option>
                                            @endforeach
                                        @endif
                                    </select>
                                </div>
                                ...
                                <div class="col-md-6 col-sm-12 pb-2">
                                    ...
                                    <select required disabled class="select2bs4 form-control"  name="service_id"   id="">
