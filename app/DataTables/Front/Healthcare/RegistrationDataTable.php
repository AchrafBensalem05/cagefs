...
class RegistrationDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ->addColumn('actions', fn($record) => "<a class='confirm-text' href='" . route('healthcare.delete_registration' , $record->id) ."' ><img src='" .asset('patient/assets/img/icons/delete.svg') . "' alt='img'></a>")
    ...
    {
        return RegistrationFilter::filters(["started_at"] ,Auth::guard('healthcare')->user()->registrations()->getQuery() , request())->orderBy('created_at' , 'desc');
    }
    ...
    {
        return $this->builder()
                    ->setTableId('my-registration-table')
                    ->columns($this->getColumns())
                    ->ajax(
                        [
                            "url"  => route("healthcare.get_my_registrations_data"),
