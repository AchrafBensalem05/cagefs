...
class DiseaseDataTable extends DataTable
{
        ...
        return (new EloquentDataTable($query))
            ->addColumn('actions', fn($record) => "  ".
                (Auth::user()->can('user-edit') ?
                    "<a href='".route("diseases.edit" , $record->id)."' class='btn btn-info btn-sm mr-1'><i class='fas fa-pencil-alt'></i>Edit</a>" : "").
                (Auth::user()->can('user-delete') ?
                    "<a href='".route("diseases.show" , $record->id)."' class='btn btn-sm btn-danger'><i class='fas fa-trash'></i>Delete</a>" : "") .
                " ")
    ...
    {
        return $this->builder()
                    ->setTableId('disease-table')
                    ->columns($this->getColumns())
                    ->ajax([
                        "url"  => route("diseases.get_index")
                    ])->orderBy(1);
    }
